...
def main():
    ...
    if digest_focus:
        n_before = sum(1 for x in processed_data if x is not None)
        processed_data = [
            item
            for item in processed_data
            if item is not None
            and (item.get("AI") or {}).get("digest_theme_relevant") is True
        ]
        print(
            f"digest_focus filter: kept {len(processed_data)}/{n_before} (digest_theme_relevant=True)",
            file=sys.stderr,
        )

    max_saved = 300
    to_save = [item for item in processed_data if item is not None]
    if len(to_save) > max_saved:
        print(f"output cap: {len(to_save)} -> {max_saved}", file=sys.stderr)
        to_save = to_save[:max_saved]

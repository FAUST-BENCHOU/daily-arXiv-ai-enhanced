...
5. **[Optional] Daily email digest (separate workflow)**  
   Email sending is handled by `.github/workflows/send-email-digest.yml`, not by the crawl workflow. Configure the following **repository Secrets** (Actions → Secrets and variables → Actions):  
   - `SMTP_HOST`, `SMTP_PORT` (e.g. `465` for SMTPS or `587` for STARTTLS)  
   - `SMTP_USER`, `SMTP_PASS` (many providers require an app-specific password)  
   - `MAIL_FROM`, `MAIL_TO`  
   The digest script reuses `OPENAI_API_KEY`, `OPENAI_BASE_URL`, and the Variables `LANGUAGE`, `MODEL_NAME` (same as paper enhancement).  
   **When it runs:**  
   - Automatically after workflow **`arXiv-daily-ai-enhanced`** finishes successfully (it reads `data/YYYY-MM-DD_AI_enhanced_<LANGUAGE>.jsonl` from the **`data`** branch). If there was no new content that day, that file may not exist and this job **skips** sending mail.  
   - Manually: Actions → **send-email-digest** → **Run workflow**; you may set **digest_date** (`YYYY-MM-DD` UTC) or leave it empty for today (UTC).  
   **SMTP TLS:** The workflow uses `secure: true` (typical for port **465**). If your provider uses port **587** with STARTTLS, you may need to change `secure` in `send-email-digest.yml` per your provider’s docs.
